...
class MsresidentFormEdit extends StatefulWidget {
  const MsresidentFormEdit({super.key});

  @override
  State<MsresidentFormEdit> createState() => _MsresidentFormEditState();
}

class _MsresidentFormEditState extends State<MsresidentFormEdit> {
  bool isFirstEdit = true;
  final arguments = Get.arguments as Map<String, dynamic>;
  dynamic resData = '';
  String _formTitle = 'Ubah Data';
  ...
  Future<void> _fetchData() async {
    ...
    if (response['xStatus'] == '1') {
      setState(() {
        ...
        controller.idCardNumber.text = resData['IDCardNumber'];
        controller.name.text = resData['Name'];
        controller.birthPlace.text = resData['BirthPlace'];
        controller.birthDay.text = resData['StrBirthDay'];
        // controller.gender.text = resData['Gender'];
        selectedGender = resData['Gender'];
        controller.province.text = resData['ProvinceName'];
        selectedProvince = resData['Province'];
        selectedProvinceName = resData['ProvinceName'];
        _fetchRegencies(selectedProvince!);
        controller.regency.text = resData['RegencyName'];
        selectedRegency = resData['Regency'];
        selectedRegencyName = resData['RegencyName'];
        controller.district.text = resData['DistrictName'];
        selectedDistrict = resData['District'];
        selectedDistrictName = resData['DistrictName'];
        controller.village.text = resData['VillageName'];
        selectedVillage = resData['Village'];
        selectedVillageName = resData['VillageName'];
        controller.address.text = resData['Address'];
        controller.religion.text = resData['Religion'];
        controller.maritalStatus.text = resData['MaritalStatus'];
        controller.employment.text = resData['Employment'];
        controller.citizenship.text = resData['Citizenship'];
        // MainDeviceUtils.hideKeyboard(context);
      });
      print(resData);
      // print(provinces);
                ...
                child: Column(
                  children: [
                    ...
                    _image != null
                        ...
                        : 
                        ((resData).isNotEmpty && resData['FileURL'].toString().length > 0) 
                        ? CachedNetworkImage(
                          imageUrl: resData['FileURL'],
                          width: MainHelperFunction.screenWidth() * 0.6,
                          height: MainHelperFunction.screenWidth() * 0.6 * 0.7,
                          fit: BoxFit.cover,
                    ...
                    SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () => controller.store(false, arguments['id'].toString(),_token, _id, _image, selectedProvince, selectedRegency, selectedDistrict, selectedVillage, selectedGender, _selectedDate != null ? DateFormat("yyyy-MM-dd").format(_selectedDate!): resData['BirthDay']), child: const Text("Update")),),
